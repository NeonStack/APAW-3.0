<script>
  ...
  $: filteredCities = $weatherData.data
    .filter(city => {
      if (selectedDistrict === 'all') return true;
      // Fix: Get district info from the metroManilaCities array
      const cityInfo = metroManilaCities.find(c => c.id === city.city_id);
  ...
  // Function to get district for a city
  function getCityDistrict(cityId) {
    const city = metroManilaCities.find(c => c.id === cityId);
  ...
  onMount(async () => {
    if ($weatherData.data.length === 0 && !$weatherData.loading) {
      await fetchWeatherData();
    }
  });
  ...
  function getPrecipitationStatus(probability) {
    if (probability === undefined || probability === null) return { text: 'N/A', color: 'gray-500' };
...
<div class="weather-tab h-full flex flex-col">
  <div class="flex justify-between items-center mb-2">
    ...
    <div class="flex space-x-2">
      ...
      <button 
        class="text-sm flex items-center gap-1 text-white bg-blue-600 hover:bg-blue-700 py-1 px-2 rounded transition-colors"
  ...
  {#if $weatherData.lastUpdated}
    <div class="text-xs text-gray-500 mb-2">
      Last updated: {moment($weatherData.lastUpdated).format('MMM D, YYYY [at] h:mm A')}
    </div>
  {/if}
  ...
  <div class="flex flex-wrap gap-2 mb-3 text-xs">
    <div class="flex items-center">
      ...
      <select 
        ...
        class="border rounded py-1 px-1.5 text-xs bg-white"
      >
  ...
  <div class="flex-1 overflow-auto">
    {#if $weatherData.loading}
      <div class="flex flex-col items-center justify-center h-36">
        ...
        <p class="mt-2 text-gray-600 text-sm">Loading weather data...</p>
      </div>
    {:else if $weatherData.error}
      <div class="bg-gray-50 border border-gray-200 rounded p-3 text-center">
        ...
        <p class="text-gray-700 text-sm mt-1">{$weatherData.error}</p>
        <button 
        ...
        {#each filteredCities as city}
          ...
          {#if todayForecast}
            <div class="bg-white border border-gray-200 rounded shadow-sm transition-all mb-5">
              ...
              <div class="flex flex-col p-2 border-l-3 border-l-blue-500">
                <div class="flex items-center justify-between mb-1">
                  <div class="flex items-center gap-1.5">
                    <h3 class="font-medium text-gray-800">{city.city_name}</h3>
                    <div class="px-1.5 py-0.5 rounded-sm text-xs font-medium bg-gray-100 text-gray-600">
                      {getCityDistrict(city.city_id)}
                    </div>
                  </div>
                </div>
                <!-- Add date on its own line with smaller text -->
                <div class="text-xs text-gray-500">
                  <Icon icon="mdi:calendar" class="inline-block mr-0.5 text-xs" /> 
              ...
              <div class="px-2 py-1.5 border-t border-gray-100">
                ...
                <div class="flex justify-between items-center mb-2">
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <div class="flex items-baseline">
                        <span class="text-xl font-bold">{Math.round(todayForecast.max_temp_c)}°C</span>
                        <span class="text-xs text-gray-500 ml-1">/ {Math.round(todayForecast.min_temp_c)}°C</span>
                      </div>
                  ...
                  <div class="flex flex-col items-end">
                    <div class="flex items-center">
                      <Icon icon="mdi:water" class="text-blue-500 mr-1" />
                ...
                <div class="grid grid-cols-4 gap-1 text-xs mb-2">
                  ...
                  <div class="bg-gray-50 p-1 rounded">
                    ...
                    <div class="font-medium">{Math.round(todayForecast.avg_wind_speed_kmh)} km/h</div>
                  </div>
                  <div class="bg-gray-50 p-1 rounded">
                ...
                <div class="grid grid-cols-3 gap-1 text-xs">
                  <div class="bg-blue-50 p-1 rounded border border-blue-100">
                    ...
                    <div class="font-medium">{formatValue(todayForecast.total_liquid_mm, 'mm')}</div>
                  </div>
                  <div class="bg-blue-50 p-1 rounded border border-blue-100">
                  ...
                  <div class="bg-blue-50 p-1 rounded border border-blue-100">
                    ...
                    <div class="font-medium">{formatValue(todayForecast.max_wind_gust_kmh, 'km/h', 0)}</div>
                  </div>
                </div>
              </div>
              ...
              <details class="text-sm border-t border-gray-100">
                ...
                <div class="grid grid-cols-4 gap-1 p-2 bg-gray-50 text-xs">
                  {#each city.forecasts.filter(f => !isToday(f.forecast_date)) as forecast}
                    <div class="flex flex-col items-center p-1 border rounded bg-white">
                      <div class="text-gray-500">{formatDate(forecast.forecast_date)}</div>
                      <div class="my-1">
                        <Icon icon={getWeatherIcon(forecast.day_icon)} class="text-blue-500" width="20" />
                      </div>
                      <div class="font-medium">{Math.round(forecast.max_temp_c)}°</div>
                      <div class="text-blue-600 font-medium">{formatValue(forecast.total_rain_mm, 'mm')}</div>
                      <div class="text-gray-500 text-xs">{forecast.day_precipitation_probability}%</div>
                    </div>
                  {/each}
                </div>
              </details>
                ...
                <div class="p-2 bg-gray-50 text-xs">
                  <div class="grid grid-cols-2 md:grid-cols-3 gap-1">
                    ...
                    <div class="bg-white p-2 rounded border">
                      <h4 class="font-medium mb-1 text-gray-700">Temperature</h4>
                      <div class="space-y-1">
                        <div class="flex justify-between">
                          <span>Min:</span> <span>{formatValue(todayForecast.min_temp_c, '°C')}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Max:</span> <span>{formatValue(todayForecast.max_temp_c, '°C')}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Avg:</span> <span>{formatValue(todayForecast.avg_temp_c, '°C')}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>RealFeel Min:</span> <span>{formatValue(todayForecast.min_realfeel_temp_c, '°C')}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>RealFeel Max:</span> <span>{formatValue(todayForecast.max_realfeel_temp_c, '°C')}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <!-- Precipitation data -->
                    <div class="bg-white p-2 rounded border">
                      <h4 class="font-medium mb-1 text-gray-700">Precipitation</h4>
                      <div class="space-y-1">
                        <div class="flex justify-between">
                          <span>Total Liquid:</span> <span>{formatValue(todayForecast.total_liquid_mm, 'mm')}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Rain:</span> <span>{formatValue(todayForecast.total_rain_mm, 'mm')}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Ice:</span> <span>{formatValue(todayForecast.total_ice_mm, 'mm')}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Hours Precip:</span> <span>{todayForecast.total_hours_precipitation || 0} hrs</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Hours Rain:</span> <span>{todayForecast.total_hours_rain || 0} hrs</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <!-- Probabilities -->
                    <div class="bg-white p-2 rounded border">
                      <h4 class="font-medium mb-1 text-gray-700">Probabilities</h4>
                      <div class="space-y-1">
                        <div class="flex justify-between">
                          <span>Precipitation (Day):</span> <span>{todayForecast.day_precipitation_probability}%</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Precipitation (Night):</span> <span>{todayForecast.night_precipitation_probability}%</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Thunderstorm (Day):</span> <span>{todayForecast.day_thunderstorm_probability}%</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Thunderstorm (Night):</span> <span>{todayForecast.night_thunderstorm_probability}%</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <!-- Wind data -->
                    <div class="bg-white p-2 rounded border">
                      <h4 class="font-medium mb-1 text-gray-700">Wind</h4>
                      <div class="space-y-1">
                        <div class="flex justify-between">
                          <span>Avg Speed:</span> <span>{formatValue(todayForecast.avg_wind_speed_kmh, 'km/h', 0)}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Max Gust:</span> <span>{formatValue(todayForecast.max_wind_gust_kmh, 'km/h', 0)}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Day Direction:</span> <span>{todayForecast.day_wind_direction_loc || 'N/A'}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Night Direction:</span> <span>{todayForecast.night_wind_direction_loc || 'N/A'}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <!-- Humidity data -->
                    <div class="bg-white p-2 rounded border">
                      <h4 class="font-medium mb-1 text-gray-700">Humidity & Atmosphere</h4>
                      <div class="space-y-1">
                        <div class="flex justify-between">
                          <span>Min Humidity:</span> <span>{todayForecast.min_relative_humidity_percent}%</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Max Humidity:</span> <span>{todayForecast.max_relative_humidity_percent}%</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Avg Humidity:</span> <span>{todayForecast.avg_relative_humidity_percent}%</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Cloud Cover:</span> <span>{todayForecast.avg_cloud_cover_percent}%</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Evapotranspiration:</span> <span>{formatValue(todayForecast.total_evapotranspiration_mm, 'mm')}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="bg-white p-2 rounded border">
                      <h4 class="font-medium mb-1 text-gray-700">Heat Indices</h4>
                      <div class="space-y-1">
                        <div class="flex justify-between">
                          <span>Avg WetBulb Temp:</span> <span>{formatValue(todayForecast.avg_wetbulb_temp_c, '°C')}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Min WetBulb Temp:</span> <span>{formatValue(todayForecast.min_wetbulb_temp_c, '°C')}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Max WetBulb Temp:</span> <span>{formatValue(todayForecast.max_wetbulb_temp_c, '°C')}</span>
                        </div>
                        <div class="flex justify-between">
                          <span>Avg WBGT:</span> <span>{formatValue(todayForecast.avg_wbgt_c, '°C')}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  {#if todayForecast.headline_text}
                    <div class="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded">
                      ...
                      {#if todayForecast.headline_category}
                        <div class="text-yellow-600">Category: {todayForecast.headline_category} (Severity: {todayForecast.headline_severity})</div>
                      {/if}
                    </div>
                  {/if}
                </div>
