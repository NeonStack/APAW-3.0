...
export async function GET({ url, request }) {
  ...
  const model = url.searchParams.get('model') || 'rf'; // Default to random forest if not specified
  ...
  try {
    ...
    const client = await Client.connect("HunterExist2/apaw_predict_v3", {
      hf_token: VITE_HF_TOKEN // Token to access private model
    });

    // Make the prediction
    const result = await client.predict("/predict_flood_sequence", {
