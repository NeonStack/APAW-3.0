...
const CONFIG = {
	...
	// Client-side fallback recheck when cache state is stale/unknown.
	CLIENT_STALE_RECHECK_MINS: 5,
...
export async function GET({ platform }) {
	...
	const fallbackNextRefreshAt = new Date(
		now.getTime() + CONFIG.CLIENT_STALE_RECHECK_MINS * 60 * 1000
	).toISOString();
	const nextRefreshAt =
		!isCacheStale && cacheStatus?.cache_expiry_time
			? cacheStatus.cache_expiry_time
			: fallbackNextRefreshAt;

	return json({
		data: activeAdvisory?.advisory_data || null,
		meta: {
			cache_expiry_time: cacheStatus?.cache_expiry_time || null,
			next_refresh_at: nextRefreshAt,
			source_cache_stale: isCacheStale,
