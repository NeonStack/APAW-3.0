...
const NCR_LOCATIONS = [
    ...
    { name: 'Marikina', lat: 14.648060, lon: 121.104192 },
    ...
    { name: 'Malabon', lat: 14.672420, lon: 120.957245 },
    { name: 'Navotas', lat: 14.666291, lon: 120.941000 },
    ...
    { name: 'Pateros', lat: 14.545080, lon: 121.069831 },
    { name: 'Taguig', lat: 14.517084, lon: 121.057200 }
];
    ...
    try {
        for (const location of NCR_LOCATIONS) {
            ...
            if (!response.ok) {
                ...
                jobSummary.push({ location: location.name, status: 'api_error', message: response.statusText });
                continue;
            }
            ...
            console.log(`Deleting existing forecast for ${location.name} from ${startDate} to ${endDate}...`);
            const { error: deleteError } = await supabase
                .from(TARGET_TABLE)
                .delete()
                .eq('location_name', location.name)
                .gte('datetime', startDate)
                .lt('datetime', endDatePlusOne.toISOString().split('T')[0]);
            ...
            if (recordsToUpsert.length > 0) {
                ...
                jobSummary.push({ location: location.name, status: 'success', records_processed: recordsToUpsert.length });
        ...
        return json({ message: 'Weather data update job completed.', summary: jobSummary }, { status: 200 });
